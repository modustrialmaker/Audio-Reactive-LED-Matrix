...
SimplePatternList gPatterns = { blobby_flow_gaps, XYmatrix, blobby_flow, rainbow, rainbowWithGlitter, confetti, sinelon, juggle, bpm };
...
void soundmems() {                                                  // Rolling average counter - means we don't have to go through an array each time.
  ...
  if ((sample > (sampleavg + 80)) && (newtime > (oldtime + 80)) ) { // Check for a peak, which is 30 > the average, but wait at least 60ms for another.  
...
void center_burst_FHT() {
  ...
  EVERY_N_MILLISECONDS(25) {
    fadeToBlackBy(leds, NUM_LEDS, 5);                            // 8 bit, 1 = slow, 255 = fast    
    sndwave();
    soundmems();
  }
